...
use crate::runner::runner::{GasRunner, Runner};
...
impl GasRunner for StatelessAptosRunner {
    fn execute_with_gas(
        &mut self,
        ...
        self.log_line(&format!(
            "TRACE: {}::{}({:?})",
            self.target_module, function_name, args
        ));
...
impl Runner for StatelessAptosRunner {
    fn execute(
        ...
        match self.execute_with_gas(inputs) {
            Ok((cov, _gas)) => Ok(cov),
            Err((cov, err)) => Err((cov, err)),
        }
    }
